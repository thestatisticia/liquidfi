...
function StreamFiDapp({ defaultTab = 'create' }) {
  ...
  // Contract address - StreamFi deployed contract on Ethereum Sepolia
  const [contractAddress, setContractAddress] = useState('0x5B6b899C15fCaa0A863936aE35A5c31f642330Dd');
  ...
  const initializeContract = async () => {
    try {
      ...
      // Check if we're on the correct network (Ethereum Sepolia) with retry
      const network = await retryWithBackoff(async () => {
        return await provider.getNetwork();
      }, 3, 2000);
      
      const expectedChainId = 11155111n; // Ethereum Sepolia
      if (network.chainId !== expectedChainId) {
        setError(`Wrong network! Please switch to Ethereum Sepolia (Chain ID: ${expectedChainId}). Current: ${network.chainId}`);
        return;
      }
      ...
      if (code === '0x' || code === '0x0') {
        setError(`No contract found at address ${contractAddress}. Please verify the contract is deployed on Ethereum Sepolia.`);
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (callError.code === 'CALL_EXCEPTION' || callError.message?.includes('missing revert data')) {
          ...
          try {
            ...
            if (code === '0x' || code === '0x0') {
              setError(`Contract not found at ${contractAddress}. Please verify deployment on Ethereum Sepolia.`);
              return;
            }
          } catch {
          ...
          setError('Failed to connect to contract. Please check your network connection and ensure you are on Ethereum Sepolia.');
          return;
        }
        throw callError;
      }
      ...
      const streamPromises = streamIds.map(async (id) => {
        try {
          ...
          // 3. All funds have been distributed (within 0.0001 TST tolerance)
      ...
      if (err.code === 'CALL_EXCEPTION' || err.message?.includes('missing revert data')) {
        errorMessage = 'Contract call failed. Please ensure you are connected to Ethereum Sepolia network and the contract is deployed.';
      } else if (err.message) {
    ...
    try {
      ...
      for (const recipient of validRecipients) {
        ...
        if (recipient.amount && recipient.amount.trim() !== '') {
          // Use individual hourly rate - treat as TST per hour
...
<body>
  ...
  <div class="section">
    ...
    ${streamData.recipientsWithRates.map((r, idx) => `
      <div class="info-row" style="padding-left: 20px;">
        ...
        <span class="value">${parseFloat(r.hourlyRate).toFixed(4)} TST/hour</span>
      </div>
    `).join('')}
      `
      : `
    <div class="info-row">
      ...
      <span class="value">${parseFloat(streamData.hourlyRate).toFixed(4)} TST/hour</span>
    </div>
      `
    }
    <div class="info-row">
  ...
  <div class="section">
    ...
    <div class="recipients-list">
      ${streamData.recipientsWithRates && streamData.recipientsWithRates.length > 0
        ? streamData.recipientsWithRates.map((r, idx) => `
          <div class="recipient-item">${idx + 1}. ${r.address} <strong>(${parseFloat(r.hourlyRate).toFixed(4)} TST/hour)</strong></div>
        `).join('')
  ...
  <div class="section">
    ...
    <div class="info-row">
      ...
      <span class="value">${parseFloat(streamData.totalFunded).toFixed(4)} TST</span>
    </div>
    ${streamData.recipientsWithRates && streamData.recipientsWithRates.length > 0 && 
      streamData.recipientsWithRates.some(r => parseFloat(r.hourlyRate) !== parseFloat(streamData.hourlyRate))
      ? `
    ...
    ${streamData.recipientsWithRates.map((r, idx) => {
      ...
      <div class="info-row" style="padding-left: 20px;">
        ...
        <span class="value">${parseFloat(r.hourlyRate).toFixed(4)} TST/h × ${streamData.duration}h = ${recipientTotal.toFixed(4)} TST</span>
      </div>
      `;
    }).join('')}
      `
    ...
    <div class="info-row">
      ...
      <span class="value">${parseFloat(streamData.hourlyRate).toFixed(4)} TST/hour × ${streamData.duration} hours × ${streamData.recipients.length} recipients</span>
    </div>
      `
    }
  </div>
  ...
  <div class="total">
    Total Payment: ${parseFloat(streamData.totalFunded).toFixed(4)} TST
  </div>

  <div class="footer">
    <p>This is a digital receipt for your StreamFi payment stream funding.</p>
    <p>Transaction verified on Ethereum Sepolia Testnet</p>
    ...
    try {
      ...
      if (hasIndividualAmounts) {
        for (const recipient of validRecipients) {
          ...
          // Treat recipient.amount as hourly rate (TST per hour)
          const hourlyRateWei = ethers.parseEther(recipient.amount);
          if (hourlyRateWei <= 0n) {
    ...
    return (
      <div className="payment-dapp">
        ...
        <div className="connect-wallet">
          <h2>StreamFi</h2>
          <p>Connect your wallet to start streaming payments</p>
          ...
          {isConnected && (
            <div className="wallet-connection">
              <div className="wallet-display">
                ...
                <span className="wallet-balance">{parseFloat(balance).toFixed(4)} TST</span>
              </div>
            </div>
          )}
        </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        ...
                        <div>
                          <label style={{ 
                            ...
                            Hourly Rate (TST/hour)
                          </label>
                          <input
                            type="number"
                            value={recipient.amount}
                            onChange={(e) => updateRecipient(index, 'amount', e.target.value)}
                            placeholder={hourlyRate ? `Default: ${hourlyRate} TST/hour` : 'Enter hourly rate'}
                            step="0.01"
                            min="0"
                            style={{ 
                              width: '100%', 
              ...
              {streamPreview && streamPreview.isValid && (
                <div style={{ 
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '18px' }}>
                    {streamPreview.hourlyRate !== null && (
                      <div>
                        ...
                        <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: 'var(--success)' }}>{streamPreview.hourlyRate.toFixed(4)} TST/hour</div>
                      </div>
                    )}
                    <div>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--success)' }}>{parseFloat(streamPreview.totalAmount).toFixed(4)} TST</div>
                    </div>
                  </div>
                        ...
                        {streamPreview.recipientAmounts.map((totalAmount, idx) => {
                          ...
                          return (
                            <li key={idx} style={{ marginBottom: '4px' }}>
                              Recipient {idx + 1}: {parseFloat(recipientHourlyRate).toFixed(4)} TST/hour = {parseFloat(totalAmount).toFixed(4)} TST total
                            </li>
                          );
                        })}
                      </ul>
                  ...
                  <div style={{ marginTop: '20px', padding: '14px', backgroundColor: 'var(--bg-card)', borderRadius: 'var(--radius-md)', fontSize: '1.2rem', border: '1px solid var(--border)' }}>
                    <strong style={{ fontFamily: 'var(--font-display)' }}>Total:</strong> {parseFloat(streamPreview.totalAmount).toFixed(4)} TST
                  </div>
                    ...
                    <div key={stream.id} className="stream-card">
                      ...
                      {stream.isActive && (
                        <div style={{ marginBottom: '15px', padding: '12px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '1.1rem' }}>
                            ...
                            <span>{totalDistributed.toFixed(4)} / {totalFunded.toFixed(4)} TST</span>
                          </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px', fontSize: '1rem', color: '#666' }}>
                            <span>Remaining: {remaining.toFixed(4)} TST</span>
                            {timeRemaining && (
                              <span>
                      ...
                      <div className="stream-details">
                            ...
                            {(() => {
                              ...
                              if (streamRates && Object.keys(streamRates).length > 0) {
                                ...
                                const ratesList = Object.entries(streamRates).map(([addr, rate]) => 
                                  `${addr.slice(0, 6)}...${addr.slice(-4)}: ${parseFloat(rate).toFixed(4)} TST/h`
                                ).join(', ');
                                return ratesList.length > 50 ? 'Multiple rates (see recipients)' : ratesList;
                              }
                              return `${parseFloat(stream.hourlyRate).toFixed(4)} TST/hour`;
                            })()}
                          </span>
                        </div>
                        <div className="detail">
                        ...
                        <div className="detail">
                          ...
                          <span className="value">{parseFloat(stream.totalFunded).toFixed(4)} TST</span>
                        </div>
                        <div className="detail">
                              ...
                              <span style={{ textDecoration: isRemoved ? 'line-through' : 'none' }}>
                                ...
                                {individualRate && (
                                  <span style={{ marginLeft: '5px', color: 'var(--success)', fontSize: '10px', fontWeight: 'bold' }}>
                                    ({parseFloat(individualRate).toFixed(2)} TST/h)
                                  </span>
                                )}
                    ...
                    <div key={stream.id} className="stream-card">
                      ...
                      <div style={{ marginBottom: '15px', padding: '14px', background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%)', border: '1px solid var(--primary)', borderRadius: 'var(--radius-md)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '1.2rem' }}>
                          <span style={{ fontWeight: 'bold', color: 'var(--primary)', fontFamily: 'var(--font-display)' }}>Your Accumulated: {accumulated.toFixed(4)} TST</span>
                          {timeRemaining && (
                      ...
                      <div className="stream-details">
                        <div className="detail">
                          ...
                          <span className="value" style={{ fontWeight: 'bold', color: '#1976d2', fontSize: '16px' }}>{accumulated.toFixed(6)} TST</span>
                        ...
                        <div className="detail">
                          ...
                          <span className="value">
                            {(() => {
                              ...
                              if (individualRate) {
                                return `${parseFloat(individualRate).toFixed(4)} TST/hour (your rate)`;
                              }
                              return `${parseFloat(stream.hourlyRate).toFixed(4)} TST/hour`;
                            })()}
                          </span>
                      ...
                      <button
                        ...
                        className="action-btn"
                      >
                        {loading ? 'Claiming...' : `Claim ${accumulated.toFixed(4)} TST`}
                      </button>
                    </div>
                  );
                })}
          ...
          <div className="analytics-dashboard">
            ...
            <div style={{ marginBottom: '40px', padding: '24px', background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(139, 92, 246, 0.1) 100%)', borderRadius: 'var(--radius-xl)', border: '2px solid var(--primary)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginTop: '4px' }}>TST</div>
                </div>
                <div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginTop: '4px' }}>TST ({overallStats.distributionRate.toFixed(1)}%)</div>
                </div>
                <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '35px' }}>
              ...
              <div style={{ padding: '28px', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(5, 150, 105, 0.15) 100%)', borderRadius: 'var(--radius-xl)', border: '2px solid var(--success)', boxShadow: 'var(--shadow-md)' }}>
                ...
                <div style={{ fontSize: '1.1rem', color: 'var(--text-secondary)', marginTop: '10px' }}>TST</div>
              </div>
              ...
              <div style={{ padding: '28px', background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.15) 0%, rgba(217, 119, 6, 0.15) 100%)', borderRadius: 'var(--radius-xl)', border: '2px solid var(--warning)', boxShadow: 'var(--shadow-md)' }}>
                ...
                <div style={{ fontSize: '1.1rem', color: 'var(--text-secondary)', marginTop: '10px' }}>TST ({analytics.distributionRate.toFixed(1)}%)</div>
              </div>
                          ...
                          tooltip: {
                            callbacks: {
                              label: function(context) {
                                ...
                                const percentage = total > 0 ? ((value / total) * 100).toFixed(1) : 0;
                                return `${value.toFixed(2)} TST (${percentage}%)`;
                              }
                            }
                          }
                        },
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '18px', marginBottom: '30px' }}>
              <div style={{ padding: '20px', backgroundColor: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                ...
                <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: 'var(--text-primary)' }}>{analytics.avgStreamValue.toFixed(2)} TST</div>
              </div>
            ...
            <div style={{ color: 'var(--text-primary)' }}>
              ...
              <div style={{ background: 'var(--bg-input)', padding: '20px', borderRadius: 'var(--radius-md)', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
                  ...
                  <span style={{ color: 'var(--text-primary)' }}>Ethereum Sepolia Testnet</span>
                </div>
              </div>
                ...
                {receiptData.streamData.recipientsWithRates && receiptData.streamData.recipientsWithRates.length > 0 && 
                 receiptData.streamData.recipientsWithRates.some(r => parseFloat(r.hourlyRate) !== parseFloat(receiptData.streamData.hourlyRate)) ? (
                    ...
                    <div style={{ marginTop: '8px', paddingLeft: '10px' }}>
                      {receiptData.streamData.recipientsWithRates.map((r, idx) => (
                        <div key={idx} style={{ padding: '4px 0', fontSize: '0.9rem' }}>
                          ...
                          <span style={{ color: 'var(--text-primary)', marginLeft: '8px' }}>{parseFloat(r.hourlyRate).toFixed(4)} TST/hour</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <span style={{ color: 'var(--text-primary)' }}>{parseFloat(receiptData.streamData.hourlyRate).toFixed(4)} TST/hour</span>
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    ...
                    receiptData.streamData.recipientsWithRates.map((r, idx) => (
                      <div key={idx} style={{ padding: '5px 0', fontFamily: 'var(--font-mono)', fontSize: '0.9rem', color: 'var(--text-primary)' }}>
                        ...
                        <span style={{ marginLeft: '10px', color: 'var(--success)', fontWeight: 'bold' }}>
                          ({parseFloat(r.hourlyRate).toFixed(4)} TST/hour)
                        </span>
                      </div>
                    ))
                  ) : (
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ color: 'var(--text-primary)' }}>{parseFloat(receiptData.streamData.totalFunded).toFixed(4)} TST</span>
                </div>
                {receiptData.streamData.recipientsWithRates && receiptData.streamData.recipientsWithRates.length > 0 && 
                 receiptData.streamData.recipientsWithRates.some(r => parseFloat(r.hourlyRate) !== parseFloat(receiptData.streamData.hourlyRate)) ? (
                        ...
                        return (
                          <div key={idx} style={{ padding: '4px 0', fontSize: '0.9rem' }}>
                            ...
                            <span style={{ color: 'var(--text-primary)', marginLeft: '8px' }}>
                              {parseFloat(r.hourlyRate).toFixed(4)} TST/h × {receiptData.streamData.duration}h = {recipientTotal.toFixed(4)} TST
                            </span>
                          </div>
                        );
                      })}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
                    ...
                    <span style={{ color: 'var(--text-primary)' }}>
                      {parseFloat(receiptData.streamData.hourlyRate).toFixed(4)} TST/hour × {receiptData.streamData.duration} hours × {receiptData.streamData.recipients.length} recipients
                    </span>
                  </div>
                )}
              </div>
              ...
              <div style={{ background: 'var(--success)', color: 'white', padding: '15px', borderRadius: 'var(--radius-md)', textAlign: 'center', fontSize: '1.2rem', fontWeight: 'bold', marginTop: '20px' }}>
                Total Payment: {parseFloat(receiptData.streamData.totalFunded).toFixed(4)} TST
              </div>
            </div>
